...
self.addEventListener("activate", e => e.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k.startsWith("flystock-") && k !== CACHE_NAME) /* shared github.io origin: only delete FlyStock's own old caches */.map(k => caches.delete(k)))).then(() => self.clients.claim())));
